...
<body>
	...
	<div id="main" class="container-fluid">
		...
		<table class="table">
			<tr>
				<td>
					<div>
						<h4>${companyTarget.companyname}  ${companyTarget.period}  Target</h4>
					</div>
			        <div>
			...
			<tr>	
                <c:forEach var="listBranchTarget" items="${branchTarget}" varStatus="status">
                	<td>
						<div>
							<h4>${listBranchTarget.branchname}  Target</h4>
						</div>
				        <div>
				    		<h5>Prospect : ${listBranchTarget.prospect}</h5>
				    		<h5>Test Drive : ${listBranchTarget.testdrive}</h5>
				    		<h5>Closed : ${listBranchTarget.closed}</h5>    		
				        </div>
                	</td>
				</c:forEach>
			</tr>
			<tr>	
                <c:forEach var="listTeamTarget" items="${teamTarget}" varStatus="status">
                	<td>
						<div>
							<h4>${listTeamTarget.teamname}  Target</h4>
						</div>
				        <div>
				    		<h5>Prospect : ${listTeamTarget.prospect}</h5>
				    		<h5>Test Drive : ${listTeamTarget.testdrive}</h5>
				    		<h5>Closed : ${listTeamTarget.closed}</h5>    		
				        </div>
                	</td>
				</c:forEach>
			</tr>
       	...
       	<div align="center">
			<div class="box box-color box-bordered">
				<div class="box-title">
					<h3>${companyTarget.companyname} ${companyTarget.period} Monthly Summary</h3>
				</div>
						...
						<tbody>
			                <c:forEach var="listSummary" items="${listUserMonthlySummary}" varStatus="status">
				                <tr>
				                    ...
				                    <td>${listSummary.branchname}</td>
				                    <td>${listSummary.teamname}</td>
	...
	$(document).ready(function() {
		if ($(".usertable").length > 0) {
            ...
            sDom: "lfrtip",
            aoColumnDefs: [{ bSortable: !1, aTargets: [0, 15]}]
			};
	        ...
	        d.columnFilter({
	            ...
	            aoColumns: [null, { type: "text" }, { type: "text" }, { type: "text" }, null, null, null, null, null, null, null, null, null, { type: "text" }, {type: "text" }, null]
	        });
		};
	})
	</script> 
